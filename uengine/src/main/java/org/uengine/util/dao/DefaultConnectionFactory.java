...
public class DefaultConnectionFactory implements ConnectionFactory {
	...
	public DataSource getDataSource() {
		...
		try {
			/*
			 * InitialContext ctx = new InitialContext(); ds = (DataSource)
			 * ctx.lookup(GlobalContext.DATASOURCE_JNDI_NAME);
			 */
			...
			bds.setMaxTotal(GlobalContext.JDBC_MAX_ACTIVE);
			bds.setMaxIdle(GlobalContext.JDBC_MAX_IDLE);
			bds.setMaxWaitMillis(GlobalContext.JDBC_MAX_WAIT);
			ds = bds;
		} catch (Exception e) {
//			e.printStackTrace();
//			BasicDataSource bds = new BasicDataSource();
