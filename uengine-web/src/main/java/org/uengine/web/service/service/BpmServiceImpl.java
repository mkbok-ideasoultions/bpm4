...
public class BpmServiceImpl implements BpmService {
	...
	public ServiceVO executeService(ServiceVO serviceVO, HttpServletRequest request) throws UEngineException {
		...
		try {
			...
			if (actionCode.equals(ACTION_CODE_COMELETE) || actionCode.equals(ACTION_CODE_SAVEONLY)
				...
				if (instanceId == null) { // 신규업무 일 경우
					if (isSubProcess) { // 서브프로세스 일 경우
						...
						String defVerId = processManagerBean.getProcessDefinitionProductionVersionByAlias(processCode); // 프로세스정의버전
																														// ID
			...
			/*
			 * 작성일: 2018-06-29 작성자: 박종훈 작성내용: BPMUtil로 넘어온 SystemActivity 완료데이터 처리
			 */
			...
			// saveComment(instance, actionCode, userId, comment, workHumanActivity); //결재의견
			// 세팅
			serviceVO.setCurrentActivities(getCurrentActivities(instance)); // 현재액티비티정보 세팅
			processManagerBean.applyChanges();
			serviceVO.setMessage(getErrorMessagefByErrorCode(UEngineException.SUCCESS_CODE, processManagerBean)); // 처리성공메시지
																													// 세팅
...
//				if ( pvv != null )
	...
	private HumanActivity rejectWorkItem(ProcessInstance instance, String userId) throws UEngineException, Exception {
		...
		if (workAct.getStatus(instance).equals(Activity.STATUS_COMPLETED)) {
			 ...
			 * 
			 * // 반려여부 DB 컬럼에 삽입 TransactionListener tl = new TransactionListener() {
			 * 
	...
	private HumanActivity collectWorkItem(ProcessInstance instance, String userId, String statusCode)
		...
		try {
			...
			if (nextHumAct == null) {
				 ...
				 * 
				 * // 회수여부 DB 컬럼에 삽입 TransactionListener tl = new TransactionListener() {
				 * 
				 * @Override public void beforeRollback(TransactionContext tx) throws Exception
				 * {
				 * 
	...
	private HumanActivity completeWorkItem(ProcessInstance instance, boolean isNew, String userId, String statusCode,
		...
		if (isNew) {
			...
			if (workHumanActivity != null) {
				...
				// workHumanActivity가 EventHumanActivity 타입이면서 workHumanActivity에 부모가
				// AllActivity 타입 일 경우 attatchSubProcess() 처리
	...
	public WorkListVO getWorkListByUserId(String userId, int pageSize, String minRegDate) throws Exception {
		...
		try {
			...
			sql.append(
					"  AND (wl.endpoint = ?  -- 아이디 입력                                                                                                	  	  \n");
			sql.append(
					"  OR rm.endpoint   = ?) -- 아이디 입력                                                                                    		  	  \n");
			...
			while (rs.next()) {
				...
				if (UEngineUtil.isNotEmpty(rs.getString("CURRSTATUSNAMES"))) {
					...
					if (statuses.length > 1) {
						status = statuses[0] + " 외 " + Integer.toString(statuses.length - 1);
				...
				if (UEngineUtil.isNotEmpty(rs.getString("CURRRSNM"))) {
					...
					if (names.length > 1) {
						name = names[0] + " 외 " + Integer.toString(names.length - 1);
	...
	public void approve(String userId, String processCode, String bizKey, Map paramMap, HttpServletRequest request)
		...
		try {
			...
			String instanceId = processManagerBean.initializeProcess(defVerId); // 인스턴스 ID 생성
			ProcessInstance instance = processManagerBean.getProcessInstance(instanceId); // 인스턴스 생성
			instance.setName(bizKey); // 인스턴스 명 세팅
	...
	public void completeWork(String userId, String instId, final String taskId, HttpServletRequest request)
		...
		try {
			...
			ProcessInstance instance = processManagerBean.getProcessInstance(instId); // 인스턴스 생성
